...
function calcular() {
    if (conta !== 0 && porcentagem !== 0 && pessoas !== 0) {
        const strontGorjetaTotal = document.querySelector(".gorjeta-total > strong");
        strontGorjetaTotal.innerHTML = `R$ ${(conta * porcentagem).toFixed(2)}`;

        const totalComGorjeta = conta + (conta * porcentagem);
        const totalPorPessoa = totalComGorjeta / pessoas;
